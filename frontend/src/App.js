...
function App() {
  ...
  useEffect(() =>{
    let link=filter();
    loadBooksData(link);
  },[categories, page, author, title]);
  
  
  return (
    <div>
      <Header />
      <Main books={books} setPage={setPage} page={page} totalPages={totalPages} setAuthor={setAuthor} setCategories={setCategories} setTitle={setTitle}/>
      <Footer />
    </div>
  );
}
